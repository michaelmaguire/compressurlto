...
public class CreateMapServletImpl extends HttpServlet {
	...
	protected void service(HttpServletRequest aRequest,
		...
		// Make sure that "thetimes.co.uk " maps to same thing as
		// "thetimes.co.uk"
		longUrl = longUrl.trim();

		// If we don't check for this, there is a danger that if a user attempts
		// to create a short
		// URL for e.g. "www.bob.com", then when we return that URL in the
		// redirect, since it doesn't
		// start with http://, our server will consider it a relative redirect,
		// and the client will
